...
export default function Tienda() {
  ...
  return (
    <>
      <div className="relative bg-cover md:bg-top bg-center md:h-[2000px] h-svh overflow-scroll text-center" style={{ backgroundImage: "url('/Tienda2500x2500.jpg')" }}>
        {showMenu && (
          menu.map((item, index) => (
                ...
                {item.menu.map((category_menu) => (
                  <div key={category_menu.id}>
                    {category_menu.checked && (
                      <div className="bg-slate-900 bg-opacity-50 p-4 mx-4 mt-4 rounded-2xl md:w-96 w-11/12">
                        ...
                        <a href="https://api.whatsapp.com/send?phone=543489538212&text=Hola%20UmamiVeg.%20Tengo%20una%20consulta."
                          className="bg-green-900 w-1/2 mt-4 ml-auto mr-auto md:text-xl text-md text-white rounded-xl p-2 hover:bg-green-950"
                        >Hacer consulta</a>
                      </div>
                    )}
                  </div>
